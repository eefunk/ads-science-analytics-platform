...
# ── Constants ─────────────────────────────────────────────────────────────────
AD_FORMATS = ["sponsored_product", "sponsored_brand", "sponsored_display", "dsp_display"]
DEVICE_TYPES = ["desktop", "mobile", "tablet", "ctv"]
PLACEMENT_TYPES = ["top_of_search", "rest_of_search", "product_detail_page", "off_amazon"]
CATEGORIES = [
    "electronics", "apparel", "home_kitchen", "beauty", "sports",
    "books", "toys", "automotive", "grocery", "health",
]
...
def generate_advertisers(n: int = 500) -> pd.DataFrame:
    ...
    tiers = RNG.choice(["small", "mid", "large", "enterprise"], n, p=[0.55, 0.28, 0.12, 0.05])
    budgets = {
        "small":      (50,      500),
        "mid":        (500,     5_000),
        "large":      (5_000,   50_000),
...
# ── Campaigns ────────────────────────────────────────────────────────────────
def generate_campaigns(advertisers: pd.DataFrame, campaigns_per_adv: int = 3) -> pd.DataFrame:
    ...
    for _, adv in advertisers.iterrows():
        ...
        for _ in range(n_campaigns):
            rows.append({
                ...
                "daily_budget_usd": (adv["daily_budget_usd"] / n_campaigns).round(2),
                "target_bid_usd":   float(adv["target_cpc_usd"]) * float(RNG.uniform(0.8, 1.4)),
                "quality_score":    float(adv["quality_score"]) + float(RNG.normal(0, 0.5)),
...
def generate_auction_events(
    ...
    bid_spreads = np.where(
        filled == 1,
        (winning_bids - clearing_prices) / np.where(winning_bids > 0, winning_bids, 1.0),
        0.0,
    ).round(4)
    ...
    ecpm = (
        winning_bids
        * winning_campaigns["quality_score"].values / 10.0
    ...
    placement_ctr_boost = np.where(
        placements == "top_of_search", 1.6,
        np.where(placements == "rest_of_search", 1.0,
...
def generate_supply_events(
    ...
    fill_rates = np.array([
        RNG.beta(
            *{"top_of_search": (7, 2), "rest_of_search": (8, 2),
              "product_detail_page": (9, 1.5), "off_amazon": (6, 3)}[p]
        )
        for p in placements
    ])
...
def generate_feature_release_data(
    ...
    if n_ft > 0:
        df.loc[filled_treated, "ecpm"] = (
            df.loc[filled_treated, "ecpm"] * RNG.uniform(1.03, 1.12, n_ft)
        )
